...
중요한 순서대로 {MAX_ITEMS}개 번호만 콤마로 출력. 예: 3,7,1,12,5,9,2,8,11,4
번호 외 텍스트 없이."""
...
2. 각 아이템마다: 한 줄 요약, 핵심 방법론(2~3문장), 기존 대비 차별점, 왜 중요한가
3. 전문 용어는 영어 유지, 설명은 한국어
4. 마지막에 "오늘의 트렌드" 섹션: 연구 흐름 3가지"""
...
def summarize(items: list[dict]) -> str:
    ...
    prompt = f"""아래 {len(items)}개 아이템을 분석해 HTML 뉴스레터 본문을 작성해 주세요.
...
2. 블로그/뉴스: <div class="section" id="blogs">
   각 아이템: <div class="card blog-card"> (간결하게)

3. 오늘의 트렌드: <div class="section" id="trends">
   <div class="trend-item">으로 3가지, emoji + 굵은 제목 + 2~3문장
